...
if (guess[4] == real[4] && guess[5] == real[5] && guess[6] == real[6])
{
  ...
  {
    cout << GRNB << guess[4] << guess[5] << guess[6] << NC << endl;
    return true;
  }
  else if(guess[0] != real[0] || guess[1] != real[1] || guess[2] != real[2] && guess[4] == real[4] && guess[5] == real[5] && guess[6] == real[6])
  {
    cout << GRNB << guess[4] << guess[5] << guess[6] << NC << endl;
    return false;
  }
} 
else if (guess[4] == real[4] && guess[6] == real[6])
{
  if(guess[5] == guess[4] || guess[5] == guess[6])
  {
...
else if(guess[5] == real[5] && guess[6] == real[6])
{
   if (guess[4] == guess[5] || guess[4] == guess[6])
  {
     cout << YLWB << guess[4] << GRNB << guess[5] << guess[6] << NC << endl;
     return false;
  }
}
